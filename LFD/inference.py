...
def load_cond_from_npz(path: str, cond_keys: list[str]) -> torch.Tensor:
    ...
    for k in cond_keys:
        ...
        if k == "hrz" and "rgt" in data:
            rgt = normalize_array(data["rgt"], key="rgt")
            hrz = data["hrz"]
            arr = np.full_like(rgt, -1.0, dtype=np.float32)
            arr[hrz > 0] = rgt[hrz > 0]
        else:
            arr = data[k]                        # [H, W]
            arr = normalize_array(arr, key=k)
        print(f"[INFO] cond {k}: shape={arr.shape}, min={arr.min()}, max={arr.max()}")
...
def get_args():
    ...
    parser.add_argument("--input", type=str,
                        default="./test_data/test.npz",)
    parser.add_argument("--ckpt", type=str,
                        default="./model/LFD_test")
    ...
    parser.add_argument("--model", type=str, default="LFD_test")
