...
def save_model_jit(args, model, model_without_ddp, optimizer, loss_scaler, epoch):
    ...
    # EMA handling for JiT/SiT
    # Assuming ema_params are stored as list of truncated parameters in model_without_ddp
    # Note: model_without_ddp.net is where the actual parameters live
    # We construct a state_dict for EMA that matches the structure of model.net
    if hasattr(model_without_ddp, 'ema_params1') and model_without_ddp.ema_params1 is not None:
        ema_state_dict1 = {}
        # Get parameter names from the underlying net
        param_names = [n for n, p in model_without_ddp.net.named_parameters()]
        for i, name in enumerate(param_names):
            # Prefix 'net.' because DenoiserSiT wraps JiT as self.net
            full_name = 'net.' + name 
            ema_state_dict1[full_name] = model_without_ddp.ema_params1[i]
        to_save['model_ema1'] = ema_state_dict1
    ...
    if hasattr(model_without_ddp, 'ema_params2') and model_without_ddp.ema_params2 is not None:
        ...
        for i, name in enumerate(param_names):
            full_name = 'net.' + name
            ema_state_dict2[full_name] = model_without_ddp.ema_params2[i]
        to_save['model_ema2'] = ema_state_dict2
