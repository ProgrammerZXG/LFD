...
class VisionRotaryEmbeddingFast(nn.Module):
    def __init__(
        ...
        if num_cls_token > 0:
            ...
            self.freqs_cos = torch.cat([cos_pad, cos_img], dim=0).cpu()  # [N_cls+N_img, D]
            self.freqs_sin = torch.cat([sin_pad, sin_img], dim=0).cpu()
        else:
            self.freqs_cos = freqs.cos().view(-1, freqs.shape[-1]).cpu()
            self.freqs_sin = freqs.sin().view(-1, freqs.shape[-1]).cpu()

    def forward(self, t):
        cos = self.freqs_cos.to(t.device)
        sin = self.freqs_sin.to(t.device)
